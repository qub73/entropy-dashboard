"""
Phase 2 filter ablation -- F1/F2/F3(a,b,c)/F4 on both datasets.

Filters:
  F1: 1-bar confirmation. Defer entry on signal bar T; enter on T+1
      only if close[T+1] has moved in intended direction by
      >= 0.5 * ATR_5 (in bps). Otherwise skip.
  F2: Pre-entry volume context. Volume over last 60 bars >=
      40th pctile of last 1440-bar (24h) volume distribution.
  F3: EMA regime gate (three variants).
        ema_fast = EMA(close, 30), ema_slow = EMA(close, 150)
        normalized = (ema_fast - ema_slow) / ATR_30
        F3-a: block LONG if normalized < -0.5    (symmetric SHORT: > +0.5)
        F3-b: block LONG if normalized < -0.3    (symmetric SHORT: > +0.3)
        F3-c: block LONG if normalized <  0.0... wait, prompt said < +0.3
              F3-c: block LONG if normalized < +0.3 (requires positive alignment)
              (symmetric: SHORT blocked if normalized > -0.3)
  F4: ATR-normalized 5-bar return band. Replace raw
      `|5-bar return| in [20, 80] bps` with `|ret_5| / ATR_30 in [0.3, 1.2]`.

Configurations run (per user update + Addendum #1 structure):
  1. baseline
  2. F3a                     13. F3a + F1
  3. F3b                     14. F3b + F1
  4. F3c                     15. F3c + F1
  ...
  6. F2                      17. F3b + F2
  7. F4                      18. F3c + F2
  8. F3a + F4                19. F3a + F1 + F2 + F4
  9. F3b + F4                20. F3b + F1 + F2 + F4
  10. F3c + F4               21. F3c + F1 + F2 + F4
  11. F1 + F2
  12. F1 + F4

Datasets:
  Feb 18 - Apr 7 Pi ETH (in-sample, primary)
  Kaggle Binance ETH/USDT -- last 60 days (OOS, confirming)

Promotion rule (per Addendum #1):
  IS: knife-catcher rate <15%, PF improvement >=25%, signals >=0.7x baseline
  OOS: PF improvement >=15%, max DD <= baseline max DD
  No sub-metric >10% worse on either dataset
  If nothing passes, report closest; do not promote.

Output:
  reports/phase2_filter_ablation.json
"""
import json, os, sys
...
OUT = ALGO / "reports" / "phase2_filter_ablation.json"
...
def rolling_pct40(vol, lookback_bars=1440):
    """40th percentile of `vol` over trailing `lookback_bars`."""
...
def run_ablation(feats, extra, params, filt):
    """Run the production engine with optional filters applied.
    filt is dict: {F1, F2, F3a, F3b, F3c, F4}."""
    n = feats['n']
    mids = feats['mid']; highs = feats['high']; lows = feats['low']
    ...
    # Candidate filter: F4 replaces the raw ret-band if on
    if filt.get("F4"):
        local_params = dict(params)
        local_params["ret_low"]  = -999  # disable raw band
        local_params["ret_high"] = 999
        cands_base = set(candidate_signals(feats, H_THRESH, local_params))
        # re-apply atr-normalized band
    ...
    trades = []
    last_loss_bar = -999
    # F1 confirmation: when a bar i passes all filters, record deferred_entry
    # to evaluate on bar i+1
    pending_entry = None
    ...
    for i in range(n):
        if in_trade:
            ...
            if tp_trailing:
                ...
                if worst <= floor:
                    ...
                    exit_reason = 'tp'; exit_pnl = tp_bps
            else:
                if worst <= -sl_bps:
                    exit_reason = 'sl'; exit_pnl = -sl_bps
                elif best >= tp_bps:
                    exit_reason = 'tp'; exit_pnl = tp_bps
            ...
            if exit_reason:
                ...
                equity += realized
                trades.append({
                    "direction": direction, "pnl_bps": exit_pnl,
        ...
        # Not in trade:
        # Handle pending F1 confirmation on bar i
        if pending_entry is not None:
            pe = pending_entry
            pending_entry = None
            # confirm: close movement >= 0.5 * ATR_5 in direction
            prev_close = mids[pe["bar"]]
            cur_close  = mids[i]
            move_bps = pe["direction"] * (cur_close - prev_close) / prev_close * 10000
            thresh = 0.5 * atr_5_bps[pe["bar"]]
            if move_bps < thresh:
                # confirmation fail -- skip this signal entirely
                pass
            else:
                # fire the entry
        ...
        # F1: defer for one-bar confirmation
        if filt.get("F1"):
            pending_entry = {"bar": i, "direction": d}
            continue

        # immediate entry (no F1)
        margin = equity * 0.90; notional = margin * 10
        equity -= notional * 5.0 / 10000
        in_trade = True; entry_idx = i; entry_price = mids[i]
    ...
    if nt == 0:
        return {"trades": 0, "compound_ret_pct": 0, "max_dd": 0,
                "win_rate": 0, "pf": 0, "knife_catchers": 0,
                "knife_rate": 0, "reasons": {}, "trades_long": 0, "trades_short": 0}
...
def load_kaggle_dataset(days=60):
    ...
    # keep last N days of 1-min-resampled data
    bars_wanted = days * 1440
    # Kaggle is already ~1-minute sampled but let's resample defensively
    ...
    if "bid_depth_5" in df_1m.columns and "ask_depth_5" in df_1m.columns:
        volumes = (df_1m["bid_depth_5"].values +
                   df_1m["ask_depth_5"].values)
    extra = build_extra_features(feats, volumes)
    print(f"  Kaggle: {feats['n']} bars ({feats['n']/1440:.1f} days)", flush=True)
    return feats, extra


def filter_configs():
    """The 21 configurations to run."""
    return [
        ("baseline",         {}),
        ("F3a",              {"F3a": True}),
        ("F3b",              {"F3b": True}),
...
def pct_improvement(val, baseline, direction="up"):
...
def apply_promotion(res_is, res_oos):
    """Return (passes, reason) for a single config."""
    reasons_fail = []
    # knife-catcher rate < 15% (IS)
    if res_is["knife_rate"] >= 0.15:
        reasons_fail.append(f"IS knife_rate={res_is['knife_rate']:.2%} >= 15%")
    # signals >= 0.7x baseline (IS)
    return reasons_fail  # caller aggregates


def main():
    pi_feats, pi_extra = load_pi_dataset()
    kg_feats, kg_extra = load_kaggle_dataset(days=60)

    configs = filter_configs()
    runs = []
    print(f"\nRunning {len(configs)} configs x 2 datasets = {len(configs)*2} ablations...", flush=True)
    ...
    def evaluate(r):
        ...
        return {
            "passes": len(fails) == 0,
            "fails": fails,
            "is_pf_improvement": pf_imp_is,
            "oos_pf_improvement": pf_imp_oos,
        }
    ...
    if passing:
        ...
        # rank by (least fails, largest IS pf improvement)
        evals.sort(key=lambda e: (len(e["fails"]), -(e["is_pf_improvement"] or -999)))
        print("No configuration passes. Closest:")
        for e in evals[:5]:
            print(f"  {e['name']}: fails={len(e['fails'])}, "
                  f"IS PF+{e['is_pf_improvement']}%, "
                  f"OOS PF+{e['oos_pf_improvement']}%")
            for f in e["fails"][:3]:
                print(f"     - {f}")

    OUT.write_text(json.dumps({
        "runs": runs, "evaluations": evals,
